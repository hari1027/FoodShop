...
const AdminSingleShopView = props => {
  ...
  const addShopToMainDB = async values => {
    ...
    try {
      const response = await axios.post(
        'http://10.0.2.2:5000/Foodshop',
        values,
      );
      if (response.status === 200) {
        try {
          const url = `http://10.0.2.2:5000/FoodshopAdmin/${values._id}`;
  ...
  const updateShopInMainDB = async values => {
    delete values.shopAlreadyPresentInMainDb;
    try {
      const url = `http://10.0.2.2:5000/Foodshop/${values._id}`;
  ...
  const RejectShop = async id => {
    try {
      const url = `http://10.0.2.2:5000/FoodshopAdmin/${id}`;
