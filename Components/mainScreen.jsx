...
const MainScreen = ({navigation}) => {
  ...
  const getShopDataById = async item => {
    try {
      const response = await axios.get(`https://foodshop-server-production.up.railway.app/FoodShop/${item}`);
    ...
    if (isBookmarked === false) {
      try {
        const response = await axios.get(
          `https://foodshop-server-production.up.railway.app/getBookmarkedShops/${uniqueEmail}`,
        );
        if (response.status === 200) {
          ...
          resp.map(async data => {
            try {
              const response = await axios.get(
                `https://foodshop-server-production.up.railway.app/getShopRatingAndFeedback/${data._id}`,
              );
              if (response.status === 200) {
                console.log(
  ...
  const getShops = async () => {
    try {
      let resp = await axios.get('https://foodshop-server-production.up.railway.app/FoodShop');
      if (resp.status === 200) {
