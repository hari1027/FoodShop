...
const UpdateShopForm = (props) => {
    ...
    const validationSchema = yup.object().shape({
        shopName: yup.string()
            ...
            .matches(/^[a-zA-Z\s]+$/, 'Shop Name should not contain special characters and Numbers')
        ...
        shopAddress: yup.string()
            ...
            .matches(/^[a-zA-Z0-9\s,\/]+$/, 'Shop Address should only contain letters, numbers and forward slash and comma')
            .required('Shop Address is required'),
        shopCategory: yup.string()
            .required('Shop Category is required'),
        foodCuisines: yup.array()
            .min(1, 'Select at least one Food Cuisine'),
        shopOwnerName: yup.string()
            ...
            .matches(/^[a-zA-Z\s]+$/, 'Shop Owner Name should not contain special characters and Numbers')
            .required('Owner Name is required'),
        shopOwnerNumber: yup.string()
            .matches(/^[6-9]\d{9}$/, 'Invalid phone number. Phone Number should start with 6, 7, 8, or 9 and have 10 digits')
            .required('Owner Number is required'),
    });

    return (
        <Formik
            initialValues={{ shopName: props.shopDetails.shopName || '', shopAddress: props.shopDetails.shopAddress || '', shopCategory: props.shopDetails.shopCategory || '', foodCuisines: props.shopDetails.foodCuisines || '', shopOwnerName: props.shopDetails.shopOwnerName || '', shopOwnerNumber: props.shopDetails.shopOwnerNumber || '' }}
            ...
            {({ handleChange, handleBlur, setFieldValue, handleSubmit, values, errors, touched }) => (
                <ScrollView style={{ flex: 0.8 }}>
                            ...
                            <View style={styles.textBox}>
                                {values.foodCuisines.length < 1 ?
                                    <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <Text style={{ fontSize: 16, fontWeight: 400, marginTop: -2, color: '#999' }} onPress={() => { toggleModal() }}>
                                            Select FoodCuisines
                                        </Text>
                                        <CustomDropdownIcon />
                                    </View> :
                                    <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <Text style={{ fontSize: 16, marginTop: -2, }} onPress={() => { toggleModal() }}>{
                                            values.foodCuisines.length > 2 ?
                                                `${values.foodCuisines[0]} ${values.foodCuisines[1]} ..+${values.foodCuisines.length - 2}`
                    ...
                    <View style={styles.buttonDiv}>
                        <TouchableOpacity onPress={() => { props.onClickBackToHome() }} style={styles.button}>
